var MooTextarea = new Class({
	...
	options:{
		...
		toolbar : 'srTextEdit_1', 	//'srMain_0' if auto is false
		imgPath : 'mootextarea/images/',
		savePath: '/siteroller/classes/editor/update.php'
	},
	
	initialize: function(els, options){
		this.setOptions(options);
		MooTextarea.Buttons.self = this;
	...
	activate: function(selectors, toolbars){
		...
		var els = $$(selectors||'textarea, .mootextarea'), i=this.options.inline, l=this.options.location.substr(4,1).toLowerCase(), t=this, mta;
		
		function insertToolbar(){
			var mta = new Element('div', {'class':'srRemove srMooTextarea '+(i?'srHide':''), 'contentEditable':false }).adopt(
				 new Element('div', {'class':'srWysEditor' }),
				 new Element('textarea', {'class':'srWygEditor srHide', 'type':'text' })
			).inject(document.body);	
			t.toolbar(t.options.toolbar, mta.getElement('.srWysEditor'));
			return mta;
		}
	...
	toolbar: function(row, toolbar){
		...
		var parent = toolbar.getElement('.srR'+num) || new Element('div',{'class':'srR'+num}).inject($(toolbar));
		
		if(!parent.getElement('.'+row)){ 
			var bar = new Element('div', {'class':row}).inject(parent);
			MooTextarea.Buttons[row].each(function(val, key){
				var properties = new Hash({
					...
					events:{
						'mousedown': function(e){ 
							e.stop(); 
							t.active = this.getParent('.srWysEditor')
...
MooTextarea.Buttons = new Hash({
	...
	srAddLink_2: new Hash({
		...
		l2:{ 'type':'text',   'id':'ght', events:{ 'mousedown':function(){ MooInline.Buttons.self.getRange(); }}},
		l3:{ 'type':'submit', 'value':'add link', events:{'click':function(){MooInline.Buttons.self.setRange()}}}
	}),
